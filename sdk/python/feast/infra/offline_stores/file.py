...
class FileRetrievalJob(RetrievalJob):
    def to_df(self):
        if not isinstance(self.entity_df, pd.DataFrame):
            raise ValueError(
                f"Please provide an entity_df of type {type(pd.DataFrame)} instead of type {type(self.entity_df)}"
            )
        """Return dataset as Pandas DataFrame synchronously"""
        entity_df_event_timestamp_col = DEFAULT_ENTITY_DF_EVENT_TIMESTAMP_COL  # local modifiable copy of global variable
        if entity_df_event_timestamp_col not in self.entity_df.columns:
            datetime_columns = self.entity_df.select_dtypes(
        ...
        self.entity_df[entity_df_event_timestamp_col] = self.entity_df[
        ...
        entity_df_with_features = self.entity_df.copy()
